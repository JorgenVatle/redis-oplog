...
class RedisSubscriptionManager {
        ...
        if (!isSynthetic) {
            ...
            subscribers.forEach(redisSubscriber => {
                try {
                    ...
                    debug(
                        `[RedisSubscriptionManager] Exception while processing event: ${e.stack}`
                    );
                }
            });
        } else {
    ...
    getDoc(collection, subscribers, data) {
        ...
        if (fieldsOfInterest === true) {
            doc = collection.findOne(doc._id, { transform: null });
        } else {
            doc = collection.findOne(doc._id, { fields: fieldsOfInterest, transform: null });
        }
